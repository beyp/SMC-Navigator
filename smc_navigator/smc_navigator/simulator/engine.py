...
from smc_navigator.risk.risk_engine import compute_risk_plan
...
def build_trade_from_signal(config: dict, signal, reason_suffix: str = "", context_df: pd.DataFrame | None = None) -> Trade:
    ...
    if risk_cfg.get("enabled", False):
        rp = compute_risk_plan(signal.direction, entry, context_df if context_df is not None else pd.DataFrame(), float(config.get("starting_capital", 100.0)), risk_cfg)
        sl = _apply_spread(float(rp["stop_loss"]), signal.direction, sp, False)
        tp = _apply_spread(float(rp["take_profit"]), signal.direction, sp, False)
        size, risk_amount = float(rp["position_size"]), float(rp["risk_amount"])
    else:
        ...
        rp = {"initial_risk": abs(entry - sl), "rr_to_tp1": 1.5, "rr_to_tp2": 2.0, "rr_to_tp3": 3.0, "stop_method": "signal", "target_method": "signal"}
    rr = _rr_ratio(entry, sl, tp)
    reason = "; ".join(signal.reason) + (f"; {reason_suffix}" if reason_suffix else "")
    return Trade(str(uuid4()), signal.timestamp, config["exchange"], signal.symbol, config["timeframe"], signal.direction, entry, sl, tp, size, risk_amount, signal.confidence_score, "OPEN", None, None, None, 0.0, 0.0, 0.0, 0.0, 0, rr, reason, "|".join(signal.tags), float(rp["initial_risk"]), float(rp["rr_to_tp1"]), float(rp["rr_to_tp2"]), float(rp["rr_to_tp3"]), str(rp["stop_method"]), str(rp["target_method"]))
...
def run_backtest_for_symbol(config: dict, symbol: str, enriched_df: pd.DataFrame, journal_path: str, h1_df: pd.DataFrame | None = None, h4_df: pd.DataFrame | None = None, warmup: int = 60, max_holding_candles: int = 10, rejected_setups: list[dict] | None = None, watch_setups: list[dict] | None = None) -> list[Trade]:
    ...
    for idx in range(start_idx, len(enriched_df) - 1):
        ...
        trade = build_trade_from_signal(config, signal, reason_suffix=f"signal_index={idx}", context_df=history)
