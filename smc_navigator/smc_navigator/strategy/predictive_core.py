...
def evaluate_predictive_probabilities(htf: pd.DataFrame, ltf: pd.DataFrame | None = None) -> PredictiveProbabilities:
    if htf.empty:
    ...
    local_high = float(htf["high"].tail(60).max())
    local_low = float(htf["low"].tail(60).min())
    eq = (local_high + local_low) / 2
    last = htf.iloc[-1]
    close = float(last["close"])
    rng = max(local_high - local_low, 1e-9)

    discount_score = _norm((eq - close) / rng + 0.5)
    proximity_to_eq = _norm(1.0 - abs(close - eq) / rng)
    compression = _norm(1.0 - ((float(htf["high"].tail(20).max()) - float(htf["low"].tail(20).min())) / max(close, 1e-9)) / 0.10)
    ...
    stop_hunt = _norm((float(last["high"]) - float(htf["high"].tail(10).max())) / max(rng, 1e-9) + 0.5)
    ...
    expansion_after_compression = _norm((htf["high"].tail(3).max() - htf["low"].tail(3).min()) / max(htf["high"].tail(20).max() - htf["low"].tail(20).min(), 1e-9))

    feature_scores.update({
        "htf_discount_premium": discount_score,
        "monthly_equilibrium_proximity": proximity_to_eq,
        "volatility_compression": compression,
        "liquidity_sweeps": _norm(stop_hunt),
        "stop_hunts": _norm(stop_hunt),
        "absorption": absorption,
        "bos_choch": _norm((bos + choch) / 2),
    ...
    if discount_score > 0.58:
        tags.append(PREDICTIVE_TAGS["htf_long_zone"])
    if absorption > 0.58:
        tags.append(PREDICTIVE_TAGS["absorption"])
    if stop_hunt > 0.55:
        tags.append(PREDICTIVE_TAGS["stop_hunt"])
    if reclaim > 0.5 and bos > 0.5:
        tags.append(PREDICTIVE_TAGS["bos_reclaim"])
    if compression > 0.6 and expansion_after_compression > 0.35:
        tags.append(PREDICTIVE_TAGS["compression_breakout"])
    if trend_exhaustion > 0.6:
        tags.append(PREDICTIVE_TAGS["trend_exhaustion"])
