...
def run(config_path: str = "config.yaml") -> None:
    ...
    for symbol in config["symbols"]:
        ...
        if m15.empty:
            logger.info("Symbol %s: no candles fetched", symbol); continue
        m15i, h1i, h4i = add_indicators(m15), add_indicators(h1), add_indicators(h4)
        logger.info("Symbol %s candles fetched: total=%s first=%s last=%s", symbol, len(m15i), m15i.iloc[0]["timestamp"], m15i.iloc[-1]["timestamp"])
        ...
        trades = run_backtest_for_symbol(config=config, symbol=symbol, enriched_df=m15i, h1_df=h1i, h4_df=h4i, journal_path=str(journal_path))
