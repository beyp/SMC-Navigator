...
class TradeStats:
    ...
    average_rr: float
    average_risk_per_trade: float
    pnl_in_r: float
    expectancy_in_r: float
    winrate_by_stop_method: dict[str, float]
    winrate_by_target_method: dict[str, float]
...
def compute_trade_stats(trades: list[Trade], initial_capital: float = 100.0) -> TradeStats:
    ...
    for t in sorted_trades:
        ...
        by_stop.setdefault(getattr(t, "stop_method", "unknown"), []).append(pnl)
        by_target.setdefault(getattr(t, "target_method", "unknown"), []).append(pnl)
    ...
    avg_rr = sum(float(getattr(t, "rr_ratio", 0.0) or 0.0) for t in trades) / total_trades if total_trades else 0.0
    avg_risk = sum(float(getattr(t, "risk_amount", 0.0) or 0.0) for t in trades) / total_trades if total_trades else 0.0
    pnl_in_r = sum((float(t.pnl or 0.0) / max(float(getattr(t, "risk_amount", 0.0) or 1e-9), 1e-9)) for t in trades) if trades else 0.0
    expectancy_r = (pnl_in_r / total_trades) if total_trades else 0.0
    win_stop = {k: (sum(1 for x in v if x > 0) / len(v) * 100 if v else 0.0) for k, v in by_stop.items()}
    win_target = {k: (sum(1 for x in v if x > 0) / len(v) * 100 if v else 0.0) for k, v in by_target.items()}
    return TradeStats(total_trades, wins, losses, winrate, gross_profit, gross_loss, net, fees, avg, pf, avg, mdd, hold, by_symbol, by_dir, by_tag, by_month, by_regime, cagr, float(sharpe), float(ulcer_index), float(recovery_factor), by_year, winrate_by_score, rev_analysis, cont_analysis, avg_rr, avg_risk, pnl_in_r, expectancy_r, win_stop, win_target)
